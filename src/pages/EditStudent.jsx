...
const EditStudent = ({ loggedInUser }) => {
  ...
  useEffect(() => {
    const fetchSeats = async () => {
      try {
        const res = await axios.get("http://localhost:5000/seat", {
  ...
  const assignSeat = async () => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/seat/assign",
        {
          seatNumber: selectedSeat,
          studentId, // Replace with actual student ID
          shiftType: selectedShifts,
          adminId: loggedInUser._id, // Replace with actual admin ID
        },
        { withCredentials: true }
      );
      ...
      setSelectedShifts([]);
      setMassages("Seat assigned successfully! redirecting to seats ");
      setTimeout(() => {
        setMassages("");
      ...
      setMassages(error.response.data.error);
      setSelectedSeat(null);
      setSelectedShifts([]);
      setTimeout(() => {
        setMassages("");
 // Redirect to seats page after err to check seats status
      }, 4000);
    }
  };
        ...
        <div className="p-6 w-full min-h-[90vh] xl:w-[70vw] mx-auto   ">
          {massages && (
            <div className="massage  w-[80vw] md:w-[50vw] lg:w-[50vw] xl:w-[40vw]   bg-green-400 text-center mx-auto my-4 rounded-lg p-4   absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2  z-50 text-black">
              ...
              <p className="text-lg mb-4 px-1 py-3 bg-green-500">{massages}</p>
            </div>
          )}
         ...
         <div className="mx-auto my-10 bg-[--primary-light-color] dark:bg-[--primary-dark-color] w-[80vw] md:w-[50vw] lg:w-[50vw] xl:w-[40vw] h-[10vh] flex items-center justify-center ">
          ...
          <div className="mt-10">
            {selectedShifts.length > 0 && (
              ...
              ${seatColor} ${
                          isSelected
                            ? "ring-2 ring-green-500 bg-green-950 text-white"
