...
const SeatDetails = () => {
  ...
  const [filters, setFilters] = useState({ morning: false, afternoon: false, night: false });

  useEffect(() => {
    const fetchSeats = async () => {
      try {
        const res = await axios.get("http://localhost:5000/seat", { withCredentials: true });
        setSeats(res.data);
      } catch (error) {
  ...
  const fetchSeatDetails = async (seatNumber) => {
    try {
      const res = await axios.get(`http://localhost:5000/seat/${seatNumber}`, { withCredentials: true });
  ...
  const getSeatColor = (shifts) => {
    ...
    if (availableShifts.length === 2) {
      if (availableShifts.includes("morning") && availableShifts.includes("afternoon")) {
        return "bg-gradient-to-r from-yellow-400 to-blue-400";
      }
      if (availableShifts.includes("morning") && availableShifts.includes("night")) {
        return "bg-gradient-to-r from-yellow-400 to-gray-800 text-white";
      }
      if (availableShifts.includes("afternoon") && availableShifts.includes("night")) {
  ...
  const filteredSeats = seats.filter(seat => {
    const selectedShifts = Object.keys(filters).filter(shift => filters[shift]);
      ...
      <div className="flex flex-col md:flex-row gap-4  ">
        ...
        {selectedSeat && seatDetails && (
          <div className="w-full md:w-1/3 p-4  rounded-lg  shadow sticky top-[10vh] z-10 bg-[--primary-light-color] dark:bg-[--primary-dark-color]">
            <h3 className="text-xl font-[font1] tracking-wider  font-bold">Seat Details</h3>
            <p className="text-md  mt-2"><strong>Seat Number:</strong> {selectedSeat.seatNumber}</p>
            <div className="siftavailable bg-[--light-color] dark:bg-[--dark-color] p-4 pt-6 mt-5 rounded-lg relative"> 
            <h4 className="text-md font-semibold absolute -top-3 -left-2 bg-[--secondary-light-color] dark:bg-[--secondary-dark-color] py-1 px-2 rounded-full ">Shift Availability</h4>
            {seatDetails.shifts.map((shift) => (
              <p key={shift.shiftType}>
                <strong>{shift.shiftType.charAt(0).toUpperCase() + shift.shiftType.slice(1)}:</strong>{" "}
                {shift.studentId ? `Occupied by ${shift.userDetails?.fullname || "Unknown"}` : "Available"}
              </p>
            ))}
            </div>
          </div>
      ...
      >
        <div className="flex items-center">
          ...
          <div className="flex-1  bg-[--primary-light-color] dark:bg-[--primary-dark-color] p-4 rounded-lg">
            <h2 className="text-lg font-bold mb-4 text-center">Available Seats</h2>
            <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
              {(filteredSeats.length ? filteredSeats : seats).map((seat) => (
                <div
                  key={seat.seatNumber}
                  className={`w-12 h-12 flex items-center justify-center rounded-lg cursor-pointer ${getSeatColor(seat.shifts)}`}
