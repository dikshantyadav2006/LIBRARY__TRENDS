...
const EditUserProfile = ({setLoggedInUser}) => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/user/user/${userId}`, {
  ...
  const validate = async (name, value) => {
    ...
    if (name === "username") {
      if (!/^[A-Za-z0-9!@#$%^&*(.)_+]{6,}$/.test(value)) {
        ...
        try {
          ...
          const res = await axios.get(`http://localhost:5000/user/find-username/${value}`, {
            withCredentials: true,
          });
    ...
    if (name === "mobile") {
      if (!/^\d{10}$/.test(value)) {
        ...
        try {
          // Check mobile number uniqueness
          const res = await axios.get(`http://localhost:5000/user/find-mobile/${value}`, {
            withCredentials: true,
          });
          if (res.data.available) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`http://localhost:5000/user/user/${userId}`, user, {
        withCredentials: true,
      });
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full md:w-[50vw] xl:w-[40vw] bg-[--primary-light-color] dark:bg-[--primary-dark-color] p-6 ">
        ...
        <h4 className="font-thin tracking-wider bg-[--light-color] dark:bg-[--dark-color] py-1  px-2 absolute text-[10px] transform -translate-y-[70%] rounded-full ">Bio</h4>
          <textarea
            ...
            value={user.bio}
            onChange={handleChange}
            className="outline-none max-h-[50vh] p-2 rounded-md bg-[--secondary-light-color] dark:bg-[--secondary-dark-color]"
            maxLength="300"
          />
